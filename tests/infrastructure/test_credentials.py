...
def test_fetch_glab_cli_token_reads_top_level_token_key(tmp_path: Path):
...
def test_fetch_glab_cli_token_honors_default_location(tmp_path: Path, monkeypatch):
...
def test_fetch_glab_cli_token_returns_none_when_config_file_missing(tmp_path: Path):
    assert fetch_glab_cli_token(env={"GLAB_CONFIG_DIR": str(tmp_path)}) is None


def test_fetch_glab_cli_token_returns_none_when_token_key_absent(tmp_path: Path):
    (tmp_path / "config.yml").write_text(yaml.dump({"host": "gitlab.com"}))

    assert fetch_glab_cli_token(env={"GLAB_CONFIG_DIR": str(tmp_path)}) is None


def test_fetch_env_token_reads_gitlab_token():
    assert fetch_env_token(env={"GITLAB_TOKEN": "env-token"}) == "env-token"


def test_fetch_env_token_returns_none_when_absent():
    assert fetch_env_token(env={}) is None


def test_fetch_own_config_token_reads_token_field():
    assert fetch_own_config_token({"token": "own-config-token"}) == "own-config-token"


def test_fetch_own_config_token_returns_none_when_absent():
