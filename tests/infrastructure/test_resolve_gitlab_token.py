...
def test_glab_cli_token_wins_when_present(tmp_path, monkeypatch):
...
def test_env_token_wins_when_glab_cli_absent(tmp_path, monkeypatch):
    monkeypatch.setenv("GLAB_CONFIG_DIR", str(tmp_path))
    monkeypatch.setenv("GITLAB_TOKEN", "env-token")
...
def test_own_config_token_wins_when_others_absent(tmp_path, monkeypatch):
    monkeypatch.setenv("GLAB_CONFIG_DIR", str(tmp_path))
...
def test_returns_none_when_no_source_has_a_token(tmp_path, monkeypatch):
    monkeypatch.setenv("GLAB_CONFIG_DIR", str(tmp_path))
    monkeypatch.delenv("GITLAB_TOKEN", raising=False)
...
def _write_glab_cli_config(config_dir, token):
    import yaml
